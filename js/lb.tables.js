	...
	lbTables.prototype = {
		...
		**/
		...
		_tablesHtml:function(self,opt){
			...
			**/
			if(self.find('table:first').length == 0){
				//构造table的框架
				var table = new Array();//创建table数组
				table.push('<table cellpadding="0" cellspacing="0"');
				if(opt.width) table.push(' width="'+opt.width+'"'); //添加宽度
				if(opt.class) table.push(' class="'+opt.class+'"'); //添加样式
				table.push(' >');
				...
				function option (){//添加操作列
					...
					if(opt.editor.rowspan == true) thead.push('data-span="true"');
					thead.push('>'+opt.editor.name+'</th>');
				} 
				if(opt.editor.value == true && opt.editor.option == "first") option();
				for(var i = 0; i<opt.th.length; i++){
					thead.push('<th ');
					if(opt.th[i].class) thead.push('class="'+opt.th[i].class+'" ');
					if(opt.th[i].width) thead.push('width="'+opt.th[i].width+'" ');
					if(opt.th[i].rowspan) thead.push('data-span="true"');
					thead.push('>');
					thead.push(opt.th[i].title);
					thead.push('</th>');
				}
				if(opt.editor.value == true && opt.editor.option == "last") {
					option();
					var m = opt.sort-1;//获得序号列的排序数
				} 
				thead.push('</tr></thead>');
				table.push(thead.join(''));
				table.push('<tbody id="'+opt.id+'"></tbody></table>');
				self.append($(table.join('')));//表格插入到当前容器
				//插入编号列
			...
			**/
			...
			.done(function(json) {
				...
				$.each(json, function(i,k){
					...
					function opption(){
						editor.push('<td');
						if(opt.editor.align) editor.push(' align="'+opt.editor.align+'"');//添加对齐方式
						editor.push('>')
						for(var l = 0;l<opt.editor.btn.length;l++){//把操作列的按钮插入进来
							editor.push('<a href="javascript:;" title="'+opt.editor.btn[l].name+'" class="'+opt.editor.btn[l].class+'">'+opt.editor.btn[l].name+'</a>');
						}
						editor.push('</td>');
						tr.push(editor.join(''));
					}
					if(opt.editor.value == true && opt.editor.option == "first") opption();//添加操作列
					for(var n=0;n<k.list.length;n++){
						tr.push('<td ');
						if(opt.th[n].class) tr.push('class="'+opt.th[n].class+'" ');//添加样式
						if(opt.th[n].align) tr.push('align="'+opt.th[n].align+'" ');//添加对齐方式
						tr.push('>');
						var item = '';
						(opt.th[n].format)?(tr.push(opt.th[n].format($.trim(k.list[n]),item))):(tr.push($.trim(k.list[n])));//添加自定义函数，数据去掉空格
						tr.push('</td>');
					}
					if(opt.editor.value == true && opt.editor.option == "last") opption();//添加操作列
				...
				//添加序号列
				var m = opt.sort;
				if(opt.editor.value == true && opt.editor.option == "last") var m = opt.sort-1;
				...
				if(tbody.find('tr').length == 0){
					var th = self.find('table:first').find('thead th');
					...
					if($tr.length != 0) tbody.find('tr.kng').remove()
					for(var i =0; i<tbody.find('tr').length;i++){
						var rowIndex = tbody.find('tr').eq(i).attr('data-row');
						var idIndex = tbody.find('tr').eq(i).attr('data-id');
					...
					for(var i=0;i<json.length;i++){
						...
						if($.inArray(idData, idIndexTr) == -1){//不存在重复id
							if($.inArray(rowData, rowIndexTr) == -1){
								...
								tbody.find('tr').eq(j).removeClass('lb-col-row');
								tbody.find('tr').eq(j).before($tr[i]);
		...
		_trEo:function(tbody){//隔行变色
			tbody.find('tr').each(function() {
				...
				(m%2 == 0)?($(this).addClass('odd')):($(this).addClass('even'));
			});
		},
